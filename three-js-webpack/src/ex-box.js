...
if (WEBGL.isWebGLAvailable()) {
    ...
    //빛 추가
    const pointLight = new THREE.AmbientLight(0xFFFFFF,1); // AmbientLight object들 전 방향에서 조명을 비춤
...
addBtn.addEventListener('click', () => {
    const rack = parseInt(rackInput.value)-1;
...
// 박스용 기본 geometry & material (전역 정의)
const boxGeometry = new THREE.BoxGeometry(2, 2, 2);
const boxMaterial = new THREE.MeshStandardMaterial({ color: 0x00ff00 });
let removalInProgress = false;
let removalPhase = 0;
let removalRackIndex = 0;
var removalBox  = null;
// 각 랙 앞에 분기 벨트 (Z축 위쪽)
...
removeBtn.addEventListener('click', () => {
    ...
    if (isValidIndex(rack, level, row) && !removalInProgress && !deliveryInProgress) {
    if (!isCellOccupied(rack, level, row)) {
                alert("해당 위치에 박스가 없습니다!");
                return;
            }
               // 셀 색상 먼저 토글 (박스 제거)
                    toggleCellColor(rack, level, row, false);

                     // 제거용 박스 생성
                            removalBox = new THREE.Mesh(boxGeometry.clone(), boxMaterial.clone());
                            removalBox.position.set(
                                [-30, 0, 30][rack], // X
                                1.75,               // Y
                                (branchZ - branchLength / 2) // Z (브랜치 끝)
                            );
                            scene.add(removalBox);

                            removalInProgress = true;
                            removalPhase = 0;
                            removalRackIndex = rack;
    } else {
        alert('잘못된 입력입니다. (랙: 1~3, 행: 1~5, 열: 1~10)');
    }
});
...
const textureBaseColor = textureLoader.load('../static/img/10308149_18303710.jpg');*/
// 컨베이어 벨트 geometry
...
let branchTimer = 0;
    ...
    let moveSpeed = speed * delta;
      ...
      if (deliveryInProgress) {
          if (!branching) {
              // 메인 벨트 따라 X축 이동
              movingBox.position.x += moveSpeed;
              ...
              if (movingBox.position.z > branchEndZ) {
                  movingBox.position.z -= moveSpeed;
 ...
 if (removalInProgress && removalBox) {

    if (removalPhase === 0) {
        // Z축 앞으로 이동 (브랜치 나오는 구간)
        const targetZ = 30;
        if (removalBox.position.z < targetZ) {
            removalBox.position.z += moveSpeed;
        } else {
            removalPhase = 1;
        }
    } else if (removalPhase === 1) {
        // X축 오른쪽으로 이동 (메인 벨트 따라)
        const targetX = conveyorLength / 2;
        if (removalBox.position.x < targetX) {
            removalBox.position.x += moveSpeed;
        } else {
            // 종료: 박스 제거
            scene.remove(removalBox);
            removalBox = null;
